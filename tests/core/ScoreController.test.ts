...
describe("ScoreController", () => {
  ...
  beforeEach(() => {
    ...
    scoreCtrl = new ScoreController(gridMock, globalThis.mockLogger);
  });

  it("should initialize score to 0", () => {
    expect(scoreCtrl.getScore()).toBe(0);
  });

  it("should count connected pipes correctly", () => {
    scoreCtrl.updateScore();
    expect(scoreCtrl.getScore()).toBe(3);
  ...
  it("should only count connected pipes when one is disconnected", () => {
    ...
    scoreCtrl.updateScore();
    expect(scoreCtrl.getScore()).toBe(2);
  });
});